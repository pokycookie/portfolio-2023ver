...
import { useRefStore, useScrollStore, useWindowStore } from "../store";
...
function AboutMePage() {
  ...
  const vh = useWindowStore((state) => state.height);

  useEffect(() => {
    const element = pageREF.current;
    if (!element) return;
    const rect = element.getBoundingClientRect();
    const offset = Math.min(Math.max(-rect.top, 0), vh + 200 * txt.length);
    if (stikyREF.current && offset > 0) stikyREF.current.scrollBy(0, 100);
    if (offsetY !== offset) setOffsetY(offset);
  }, [offsetY, scroll, vh]);
  ...
  return (
    <div css={[containerCSS, { height: vh + 200 * txt.length + 500 }]} ref={pageREF}>
      <motion.div
      ...
      >
        <AnimatePresence>
          {txt.slice(0, Math.floor(offsetY / 200)).map((e, i) => {
            return (
              <TalkBubble key={i} text={e.text} opposite={e.opposite} isSeperator={e.isSeperator} />
            );
          })}
        </AnimatePresence>
      </motion.div>
...
const txt = [
  { text: "안녕하세요! 도전하는 개발자 COOKIE입니다.", opposite: true, isSeperator: false },
  { text: "반갑습니다🖐", opposite: false, isSeperator: true },
  { text: "도전하는 개발자라는 건 어떤 의미인가요?", opposite: false, isSeperator: false },
  {
  ...
  { text: "좋습니다.", opposite: false, isSeperator: true },
  { text: "간단한 자기소개를 들을 수 있을까요?", opposite: false, isSeperator: false },
  {
